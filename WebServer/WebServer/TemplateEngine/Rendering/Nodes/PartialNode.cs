namespace WebServer.TemplateEngine.Rendering.Nodes
{
    using System.Text;
    using Exceptions;
    ...
    public class PartialNode : Node
    {
        ...
        public override Node Parse(NodeList prototypes, Node parent, LineInfo line, ref int offset)
        {
            ...
            if (pos == -1)
                throw new CodeGeneratorException(line.LineNumber, line.Data,
                    "PartialNode does not contain an end paranthesis.");
        ...
        protected string GetCode()
        {
            ...
            return "hiddenTemplateManager.RenderPartial(\"" + _target + "\", args, new TemplateArguments(" +
                   argumentString + "))";
        }
        ...
        protected void ThrowError(string reason)
        {
            ...
            throw new CodeGeneratorException(LineInfo.LineNumber, LineInfo.Data,
                "PartialNode with target '" + _target + "'" + reason);
        }
    }
}
